...
// I'll just find the vodka block and replace it manually.
const target = `  , {
    url: '/spirits/vodka/',
    slug: 'vodka',
    mainCategory: 'spirits', subCategory: 'vodka',
    name: 'Vodka',
    h1: 'Vodka',
...
const startIdx = code.indexOf(`  , {    url: '/spirits/vodka/'`);
if (startIdx !== -1) {
  code = code.slice(0, startIdx) + target;
  fs.writeFileSync('src/config/seoPages.js', code);
  console.log('Fixed SEO Pages');
} else {
  console.log('Could not find vodka block');
}
